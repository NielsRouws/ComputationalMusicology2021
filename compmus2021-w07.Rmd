...
wood %>%
  ...
  geom_tile() +
  labs(title = "Euclidian", x = "Time (s)", y = NULL, fill = "Magnitude") +
  theme_minimal() +
...
wood %>%
  ...
  geom_tile() +
  labs(title = "Manhattan", x = "Time (s)", y = NULL, fill = "Magnitude") +
  theme_minimal() +
  scale_fill_viridis_c()
...
wood %>%
  ...
  geom_tile() +
  labs(title = "Chebyshev", x = "Time (s)", y = NULL, fill = "Magnitude") +
  theme_minimal() +
  scale_fill_viridis_c()
```
...
```{r}
megalovania <-
  get_tidy_audio_analysis("1J03Vp93ybKIxfzYI4YJtL") %>%
  select(segments) %>%
  unnest(segments) %>%
  select(start, duration, pitches)
...
compmus_long_distance(
  ...
  ggplot(
    aes(
      x = xstart + xduration / 4,
      width = xduration / 2,
      y = ystart + yduration / 4,
      height = yduration / 2,
      fill = d / 2
    )
  ) +
